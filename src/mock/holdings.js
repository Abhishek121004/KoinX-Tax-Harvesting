...
const normalizeHoldingForDemo = (holding) => {
  const currentValue = Math.abs(holding.totalHolding * holding.currentPrice);
  const maxGain = Math.max(Math.abs(holding.stcg.gain), Math.abs(holding.ltcg.gain));
  const targetValue = 2500;
  const targetGain = 600;
  const valueScale = currentValue > 0 ? targetValue / currentValue : 1;
  const gainScale = maxGain > 0 ? targetGain / maxGain : 1;
  const scale = Math.max(1, Math.min(500000, Math.max(valueScale, gainScale)));

  return {
    ...holding,
    totalHolding: holding.totalHolding * scale,
    stcg: {
      ...holding.stcg,
      balance: holding.stcg.balance * scale,
      gain: holding.stcg.gain * scale,
    },
    ltcg: {
      ...holding.ltcg,
      balance: holding.ltcg.balance * scale,
      gain: holding.ltcg.gain * scale,
    },
  };
};
...
export const fetchHoldings = () =>
  new Promise((resolve) => {
    setTimeout(() => {
      resolve(
        holdingsResponse.map((holding, index) => ({
          ...normalizeHoldingForDemo(holding),
